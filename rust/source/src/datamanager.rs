...
mod tests {
    ...
    async fn backup_failed() -> anyhow::Result<()> {
        ...
        // Body of the test. Panics if it times out.
        {
            ...
            // Fail right away. This is expected to finish the backup and release
            // the slot.
            let unsupported = disk::DiskError::Unsupported("Not even a file".into());
            chunk.send(Some(Err(unsupported)));

            tokio::select! {
                Some(_) = dm.backup_slots().recv() => {
                }
                _ = sleep(Duration::from_secs(1)) => {
                    panic!("timed out waiting for slot");
                }
            }
            let result = backup_done.await?;
            assert_eq!(result.path, PathBuf::from("/a"));

            if let HashUpdate::Unreadable(disk::DiskError::Unsupported(msg)) = result.update {
                assert_eq!(msg, "Not even a file");
            } else {
